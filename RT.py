...
# 水平角度が半分以上のみで検定
control_RT_half = [x['RT'] for x in control if x['RT'] is not None and x['HDegree'] > 29]
...
# 水平角度が半分未満のみも用意
control_RT_s_half = [x['RT'] for x in control if x['RT'] is not None and x['HDegree'] <= 29]
near_RT_s_half = [x['RT'] for x in near if x['RT'] is not None and x['HDegree'] <= 29]
...
#  Wilcoxonの順位和検定
CN_wil_half,CN_p_half = mannwhitneyu(control_RT_s_half,near_RT_s_half,method='exact')
CF_wil_half,CF_p_half = mannwhitneyu(control_RT_s_half,far_RT_s_half,method='exact')
...
# 半未満データ
medians = [np.median(control_RT_s_half), np.median(near_RT_s_half), np.median(far_RT_s_half)]
iqrs = [np.subtract(*np.percentile(control_RT_s_half, [75, 25])), np.subtract(*np.percentile(near_RT_s_half, [75, 25])), np.subtract(*np.percentile(far_RT_s_half, [75, 25]))]
